...
// @version      1.0
// @description  获取欧路英汉词典内容并在底部面板显示
// @author       Assistant
...
(function() {
    ...
    function initialize() {
        ...
        console.log('Eudic 脚本初始化完成（面板绑定）');
    }
    ...
    function searchEudic(query) {
        ...
        GM_xmlhttpRequest({
            ...
            onerror: function() { isSearching = false; showResult('网络错误', '无法连接到欧路词典', 'error'); },
            ontimeout: function() { isSearching = false; showResult('请求超时', '连接欧路词典超时', 'error'); }
        });
    }
    ...
    function processResponse(html, query) {
        try {
            ...
            else showResult('未找到', `在欧路词典中未找到单词 "${query}"`, 'error');
        } catch (err) {
...
a
    ...
    ">`;
        ...
        <h2 style="color:#2575fc;"><i class="fas fa-book"></i> ${escapeHtml(word)}</h2>
    </div>`;
        ...
        // ==== 去掉近义词/联想词部分 ====
        // const expSYNchild = doc.querySelector('#ExpSYNchild');
        // if (expSYNchild) { ... }

        content += addFooter('Eudic');
        content += `</div>`; // 结束外部容器
        return content;
    }
